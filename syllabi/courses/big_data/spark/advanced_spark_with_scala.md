...
---
<!-- course: advanced_spark_with_scala -->
# Advanced `Spark` with `Scala`

## Description
This course is designed for data engineers and developers who have a basic understanding of `Apache Spark` and `Scala`. It covers advanced topics in `Spark` programming using the native `Scala` `API`, focusing on performance optimization, advanced analytics, and real-world applications. Since `Spark` is written in `Scala`, this course also covers `Scala`-specific advantages such as direct access to the `Spark` internals and idiomatic functional programming patterns.

## Duration
24 hours / 3 days

## Intended Audience
* Data engineers and developers who already have a basic understanding of `Apache Spark` and `Scala`.
...
* Those who have completed a basic `Spark` with `Scala` course and are ready to move on to more complex topics.
...
* Familiarity with `Spark` core concepts and the `Scala` `Spark` `API`
...
* Leverage `Scala`-specific patterns for idiomatic `Spark` development

## Outline
<!-- chapter: advanced-rdd-and-dataset-operations, duration: 5h -->
* Advanced `RDD` and Dataset Operations
    ...
    * Leveraging `Scala` implicits and type classes for `Spark` extensions
<!-- chapter: spark-sql-and-catalyst-optimizer, duration: 4h -->
...
<!-- chapter: advanced-spark-mllib, duration: 3h -->
...
<!-- chapter: spark-streaming-and-structured-streaming, duration: 3h -->
...
<!-- chapter: advanced-spark-internals-and-best-practices, duration: 5h -->
* Advanced `Spark` Internals and Best Practices
    * Understanding `Spark` internals: DAG scheduler, task scheduler, memory management
