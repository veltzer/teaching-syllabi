...
portability and the large number of open source modules and packages `make` it very popular for
scientific use.
...
## Intended Audience
* Developers who would like to start using `Spark` using the `Python` language
...
* Gain proficiency in using `Python` for large-scale data processing with `Spark`
...
## Outline
<!-- chapter: python-crash-course-if-needed, duration: 2h -->
* `Python` Crash Course (if needed)
    * Basic Syntax
