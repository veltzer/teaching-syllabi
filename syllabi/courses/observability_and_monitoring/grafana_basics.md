---
tags:
  - tools:grafana
  - practices:monitoring
  - practices:observability
level: beginner
category: observability
duration_hours: 16
audience:
  - audiences:sysadmins
  - audiences:devops
---

<!-- course: grafana_basics -->
# `Grafana` Basics

## Description
This course provides a hands-on introduction to `Grafana`, the popular open-source platform for monitoring and observability. Participants will learn how to connect data sources, build interactive dashboards, and use panels, variables, and plugins to visualize metrics effectively.

## Duration
16 hours / 1 day

## Intended Audience
`DevOps` engineers and system administrators who need to build monitoring dashboards and visualize infrastructure and application metrics.

## Prerequisites
* Tech affinity.

## Objectives
* Integrate data sources with `Grafana` and create queries against them
* Build and customize graph panels with visualization options, fields, overrides, and transformations
* Use variables to create dynamic, reusable dashboards
* Extend `Grafana` functionality with plugins
* Apply dashboard design best practices for effective monitoring

## Outline
<!-- chapter: introduction, duration: 1h -->
* Introduction
<!-- chapter: data-source-integration, duration: 1h -->
* Data Source Integration
<!-- chapter: create-our-first-data-source, duration: 2h -->
* Create our First Data Source
<!-- chapter: graph-panel-visualization-options, duration: 2h -->
* Graph Panel : Visualization Options
<!-- chapter: graph-panel-fields-overrides, duration: 2h -->
* Graph Panel : Fields & Overrides
<!-- chapter: graph-panel-transformations, duration: 2h -->
* Graph Panel : Transformations
<!-- chapter: dynamic-graphs-from-variables, duration: 2h -->
* Dynamic Graphs from Variables
<!-- chapter: using-plugins, duration: 2h -->
* Using Plugins
<!-- chapter: dashboard-design, duration: 2h -->
* Dashboard Design

## Copyright
Mark Veltzer [mark.veltzer@gmail.com](mailto:mark.veltzer@gmail.com), © 2026
